...
def get_report(train_Y, train_preds,
               val_Y, val_preds,
               test_Y, test_preds,
               run_configuration):
    output = {
...
def main(
    ...
    # dataset_path = "D:\\conventional_ML_Research\\physionet2021_dataset"
    ...
    train_df_indices, val_df_indices = list(split_dataset_custom(train_val_df, 1, fraction_of_train))[0]
    train_df = train_val_df.iloc[train_df_indices]
    val_df = train_val_df.iloc[val_df_indices]
    ...
    shap_vals, y_test_x = get_shap_vals(
        model, 
        train_dataset.get_all_xs(), 
        np.argmax(train_dataset.get_all_ys(), axis=1), 
        val_dataset.get_all_xs(), 
        np.argmax(val_dataset.get_all_ys(), axis=1))
    print("success shap", "=" * 20)
    return run_output
...
if __name__ == "__main__":
    ...
    main(config['leads'], config['fraction_of_train'], config['balance_train_samples'], 
