...
def main(config):
    results = {}
    batches_num = [100, 300, 500, 700]
    leads = [1, 12]
    combinations = list(itertools.product(batches_num, leads))
    for iteration in range(10):
        for comb in combinations:
            train_batch_num = comb[0]
            leads = comb[1]
            config['train_batches'] = train_batch_num
            config['leads'] = [i for i in range(leads)]
            config['arrhythmia_label'] = "AF"
            results[f"{comb}"] = get_results_arrhhythm_lead(config)
        json.dump(results, open(f'{config["save_log_path"]}/optimal_batches_{iteration}.json', 'w'))

if __name__ == "__main__":
    config = read_config(f'{DIR_PATH}/configs/optimal_batches_size.json')
    main(config)