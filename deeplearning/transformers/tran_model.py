...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv1d(12, 64, kernel_size=5, stride=1, padding=2)
        ...
        self.fc1 = nn.Linear(256 * 875, 512)
        self.fc2 = nn.Linear(512, 256)
        self.fc3 = nn.Linear(256, 3)  # Output has 3 classes

    def forward(self, x):
        # Input shape: (batch_size, 12, 7000)
...
if __name__ == "__main__":
    import numpy as np
    test_data = torch.randn(12, 32, 12, 7000).to('cuda')
    test_outp = torch.randn(12, 32, 3).to('cuda')
    # model = CNN()
    model = CTN(256, 8, 2048, 8, 0.2, 64, [0, 1, 2]).to('cuda')
    opt = torch.optim.Adam(model.parameters(), lr=1e-3, betas=(0.9, 0.98), eps=1e-9)

    model.train()
    for epoch in range(30):
        losses, probs, labels = [], [], []
        for i, (x, y) in enumerate(zip(test_data, test_outp)):
            opt.zero_grad()
            out = model(x)
        ...
        print(f'Epoch {epoch} - Loss: {loss}, F1: {f1_score}')
    pass